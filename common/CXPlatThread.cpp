...
//		if you wish to ensure proper cleanup prior to destruction of
//		the object..
//
//		However, if neither has been called, and the object is deleted
//		the destructor will automatically call WaitForThreadCompletion(INFINITE).
//
...
CXPlatThread::THREAD_STATUS CXPlatThread::StartThread(void *pParam, BOOL bStartSuspended)
{
    ...
    // Start thread
#if defined(_WIN32)
    int iStartFlag = bStartSuspended ? 0 : 1;
    unsigned uThread;
